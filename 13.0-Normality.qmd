...
O teste de Shapiro-Wilk é um teste estatístico utilizado para avaliar a normalidade dos dados. A hipótese nula do teste é que os dados seguem uma distribuição normal. Ou seja, se o valor-p do teste for menor que o nível de significância, a hipótese nula é rejeitada, indicando que os dados não seguem uma distribuição normal. Por outro lado, se o valor-p for maior que o nível de significância, a hipótese nula não é rejeitada: não encontramos evidência contra a normalidade. Note a diferença: o teste não prova que os dados são normais, apenas não detectou um desvio da normalidade, o que é mais fácil de acontecer em amostras pequenas.
...
```
Observe que o valor-p foi realmente maior que o nível de significância: não há evidência contra a normalidade, como esperado.
...
O teste de Kolmogorov-Smirnov é um outro teste estatístico utilizado para avaliar a normalidade dos dados. Da mesma forma que o teste de Shapiro-Wilk, a hipótese nula do teste de Kolmogorov-Smirnov é que os dados seguem uma distribuição normal. Ou seja, se o valor-p do teste for menor que o nível de significância, a hipótese nula é rejeitada, indicando que os dados não seguem uma distribuição normal. Por outro lado, se o valor-p for maior que o nível de significância, a hipótese nula não é rejeitada: não encontramos evidência contra a normalidade. Note a diferença: o teste não prova que os dados são normais, apenas não detectou um desvio da normalidade, o que é mais fácil de acontecer em amostras pequenas.

O teste de Kolmogorov-Smirnov também pode ser usado para testar se os dados vêm de outras distribuições, pois o teste na verdade verifica se duas distribuições são iguais, ou seja, se uma amostra vem de uma distribuição em particular. Quando desejamos saber se a amostra tem uma distribuição normal, o teste de Kolmogorov-Smirnov compara a distribuição empírica dos dados com a distribuição teórica normal.
...
```
Observe que o valor de p foi realmente maior que o nível de significância: não há evidência contra a normalidade, como esperado.

Um detalhe importante: aqui informamos ao teste a média e o desvio padrão calculados da própria amostra. A rigor, o teste de Kolmogorov-Smirnov supõe que esses parâmetros sejam conhecidos de antemão; quando são estimados dos dados, o teste fica conservador (o valor-p sai maior do que deveria). A versão corrigida para esse caso é o teste de Lilliefors, disponível na função `lillie.test()` do pacote `nortest`. Na prática, para avaliar normalidade o teste de Shapiro-Wilk é preferível.
