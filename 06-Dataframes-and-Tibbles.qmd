...
Data frames e Tibbles são os objetos do R para armazenar dados. Se os vetores são a base do R, os data frames e as tibbles são o coração do R.
...
Podemos carregar os datasets já inclusos no R com a mesma função `data()`, bastando incluir como argumento o nome do dataset desejado. No exemplo a seguir iremos carregar o dataset `USArrests`. Este dataset contém estatísticas das prisões por 100.000 habitantes por assalto, assassinato e estupro em cada um dos 50 estados dos EUA em 1973, como também a porcentagem da população urbana.

```{r}
data(USArrests)
```

Com esse dataset carregado podemos agora visualizar os dados simplesmente digitando o nome do dataset no console.
...
Vejamos a estrutura de um dataset com 403 pacientes num estudo sobre diabetes, obesidade e outros fatores de risco para doenças coronarianas. Esse dataset foi obtido no site da universidade de Vanderbilt, no link a seguir, onde tem 19 variáveis. A versão usada neste capítulo é uma cópia guardada na pasta `dataset` do projeto, com os nomes das variáveis traduzidos para o português e 18 colunas (ver a lista de datasets no final do livro). O data frame contém então 403 linhas e 18 colunas.
...
A última linha acima criou o data frame, com as duas colunas (nome e idade) e as 3 linhas, uma para cada participante da pesquisa e armazenou esse data frame no objeto `staff`. Veja que o objeto `staff` é um `data.frame` do R.

```{r}
class(staff)
```

Podemos visualizar a estrutura desse data frame com a função `str(staff)`.
...
Esse data frame é um exemplo de dados no formato wide (largo). E é muitas vezes a forma usual de um conjunto de dados.
...
Sugiro a leitura do artigo **Tidy Data** de Hadley Wickham, publicado no Journal of Statistical Software, Vol. 59, Issue 10, no link a seguir: <https://vita.had.co.nz/papers/tidy-data.pdf>

## `Tibbles`, os dataframes do tidyverse

Tibbles são data frames mais amigáveis. As `tibbles` foram criadas para resolver alguns problemas dos data frames e fazem parte do pacote `tibble` do `tidyverse`. Veremos nos capítulos adiante o que são pacotes, o que é o `tidyverse`.
...
# verificando que os dados agora estão armazenados numa tibble
str(diabetes)
```
