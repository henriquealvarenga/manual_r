...
Vamos explorar a regressão logística usando o dataset `SAheart` do pacote `bestglm`. Esses dados foram coletados como parte de um estudo para entender os fatores de risco associados à doença cardíaca coronariana (CHD - Coronary Heart Disease) em uma população sul-africana, para identificar os fatores que podem contribuir para a incidência de doenças coronarianas [@Rossouw1983]. É um conjunto de dados frequentemente utilizado para ilustrar análises estatísticas e técnicas de aprendizado de máquina, especialmente no contexto de predição de doenças cardíacas.
...
A normalização das variáveis numéricas é um passo comum de pré-processamento, mas é preciso ser claro sobre o que ela faz e o que não faz. A função `glm()` do R estima os coeficientes da regressão logística por um algoritmo que **não depende da escala das variáveis**: normalizar ou não, as previsões e os valores-p saem iguais, e só os coeficientes mudam de unidade. A normalização passa a ser importante em dois casos: quando queremos **comparar a magnitude dos coeficientes** entre variáveis medidas em unidades diferentes, e quando usamos algoritmos que dependem da escala, como os métodos com regularização (lasso, ridge) e as redes neurais, muito usados em aprendizado de máquina.

Por exemplo, no nosso conjunto de dados, os valores de LDL dos pacientes variam de **0 a 15**, enquanto o consumo de álcool varia de **0 a 147 gramas por dia**. Sem normalização, o coeficiente do álcool corresponde ao efeito de **1 grama** a mais por dia, e o do LDL ao efeito de **1 unidade** a mais: os dois números não são comparáveis entre si. Depois da normalização, cada coeficiente passa a representar o efeito de **1 desvio padrão** a mais da variável, o que permite compará-los.

A normalização ajusta os valores para uma escala comum, geralmente entre 0 e 1 ou com média zero e desvio padrão um. O preço a pagar é a interpretação: os coeficientes deixam de estar nas unidades originais das variáveis.

O mesmo raciocínio vale para a regressão linear do capítulo anterior: lá também os coeficientes se ajustam à escala das variáveis, e preferimos mantê-los nas unidades originais, em que cada coeficiente representa a variação esperada no desfecho para uma unidade a mais do preditor. Neste capítulo vamos normalizar para mostrar a técnica e para poder comparar os coeficientes.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
head(SAheart) |>  kbl(caption = "Primeiras linhas do Dataset SAheart depois da normalização",
      booktabs = TRUE, 
      row.names = FALSE, 
      digits=2) |>
  kable_classic(full_width = F, html_font = "Cambria") |>
...
Como pode ser visto no gráfico acima, as variáveis adiposidade e obesidade parecem estar correlacionadas, o que pode ser mensurado com o código abaixo:

```{r}
cor.test(SAheart$adiposity, SAheart$obesity)
```

Mesmo sem esses dados, seria plausível imaginar que o nível de obesidade e de adiposidade têm uma correlação forte, sendo adequado inserir apenas uma dessas duas variáveis no modelo.
...
Na regressão linear usamos os resíduos (ou erros) para calcular a acurácia do modelo. Ou seja, calculamos o quanto o valor real se desviava do valor predito. Na regressão logística não podemos calcular os resíduos dessa forma, e não existe um $r^2$ com a mesma interpretação. Há medidas análogas, chamadas de pseudo-$R^2$ (como as de McFadden e de Nagelkerke), mas elas são menos usadas; o mais comum é avaliar o modelo pela sua capacidade de classificar corretamente, como faremos a seguir.
