...
ggplot(NULL, aes(c(lim.inf,lim.sup))) + 
    ...
    ylab("density") +
   annotate("text", x = 0, 
             y = 0.15, # old cod: dt(x.value, df = gl)
             size = 7, 
             fontface ="bold", 
             hjust=0.5, 
             label=left.area.text.decimal) +
  annotate("text", x = 0, 
             y = 0.115, # old cod: dt(x.value, df = gl)
             size = 4, 
             fontface ="bold", 
             hjust=0.5, 
             label="ou") +
  annotate("text", x = 0, 
             y = 0.08, # old cod: dt(x.value, df = gl)
             size = 7, 
             fontface ="bold", 
             hjust=0.5, 
             label=left.area.text) +
...
| Beta-Binomial     | `dbetabinom` (MASS)   | `pbetabinom` (MASS)   | `qbetabinom` (MASS)   | `rbetabinom` (MASS)   |
