...
O R, sendo uma linguagem estatística, naturalmente possui diversos comandos para criar conjuntos de números aleatórios com distribuições de diversos tipos. Para cada distribuição o R possui 4 funções principais: densidade de probabilidade (d), probabilidade cumulativa (p), quantis (q) e números randomizados (r). Cada uma dessas funções é obtida colocando-se a letra correspondente (d, p, q, r) como prefixo do nome da distribuição no R. A tabela abaixo mostra algumas das distribuições e as funções associadas.
...
```{r}
# Cada execução desse código resultará sempre na mesma sequência de 5 números.
# Alterar o argumento da função set.seed irá mudar a sequência de números.
set.seed(1)
rnorm(5)
```
...
A função `runif()` gera um conjunto randomizado de números a partir de uma distribuição uniforme. Os argumentos dessa função são: a quantidade de números, o valor mínimo e o valor máximo.
...
podemos arredondar esses valores com a função round().
...
Entretanto, à medida que aumentamos o número de elementos, a forma da distribuição se parece cada vez mais com uma distribuição uniforme. Veja no código abaixo um conjunto randomizado de 10, 100, 1000 e 10.000 elementos criados com a função `runif(`) e os histogramas dessas distribuições.
...
A função `dunif()` calcula a função de densidade de probabilidade (PDF) da distribuição uniforme. Atenção: numa distribuição contínua, a probabilidade de a variável ser exatamente igual a um valor é zero; a densidade mede o quanto a probabilidade se concentra em torno daquele valor, e é a área sob a curva de densidade (calculada por `punif()`, logo abaixo) que dá probabilidades. A função `dunif()` tem três argumentos:
...
A densidade é 0.01 em qualquer ponto entre 0 e 100, porque na distribuição uniforme ela é constante e igual a 1/(max − min) = 1/100. Como consequência, a probabilidade de a variável cair num intervalo qualquer é o comprimento do intervalo vezes 0.01: por exemplo, entre 5 e 15 é 0.10.
...
A função `qunif()` calcula a função quantil da distribuição uniforme. A função quantil é o inverso do CDF. Dada uma probabilidade como entrada, a função retorna o valor que tem essa probabilidade.
...
Ou seja, aproximadamente 1% dos homens brasileiros dessa amostra têm altura superior a 190cm.
...
A função `qnorm()` faz o cálculo inverso da função `pnorm()`. É utilizada para calcular o quantil (ou percentil) de uma distribuição normal. Isso significa que ela fornece o valor x tal que a área sob a curva normal à esquerda de x é igual a uma probabilidade especificada p. O argumento da função `qnorm(p)` é exatamente essa probabilidade, o percentual abaixo da curva à esquerda do ponto x. O resultado dessa função é o valor no eixo x que delimita essa área.

Com essa função podemos nos perguntar qual deve ser a altura das portas de um avião para que 99% dos brasileiros usuários dos aeroportos possam passar pela porta de um avião sem ter de se abaixar? Ou seja, qual a medida da estatura abaixo da qual estão 99% dos brasileiros dessa amostra? Lembrando que a média da altura dos homens brasileiros usuários do sistema de aviação foi estimada em 173,1 cm com um desvio padrão de 7,3cm, o código fica assim:
...
A distribuição t de Student (também conhecida como distribuição t) foi introduzida por William Sealy Gosset em 1908, sendo usada para estimar a média de variáveis aleatórias normalmente distribuídas para as quais o tamanho da amostra é pequeno e o desvio padrão é desconhecido. A distribuição t parece muito semelhante à distribuição normal, e converge para a curva normal à medida que o tamanho da amostra aumenta.

### `rt()`

A função `rt()` cria conjunto randomizado de números a partir de uma distribuição t de Student. Os argumentos dessa função são: a quantidade de números e os graus de liberdade (*degrees of freedom* - df).
...
A função `pt(q)` é utilizada para calcular a função de distribuição acumulada (CDF) da distribuição t de Student. Ela fornece a probabilidade de que uma variável aleatória com uma distribuição t de Student seja menor ou igual a um determinado valor x. Ou seja, calcula área abaixo da curva de distribuição t, à esquerda de q (quantil).

Os argumentos dessa função são o quantil (q) e os graus de liberdade da curva t (df). O código e o gráfico abaixo mostram o resultado da função `pt()` e a interpretação visual da função.
...
A função `qt()`, faz o inverso da função `pt()`: dada uma área abaixo da curva de distribuição t, a função retorna o quantil (q) que delimita essa área à esquerda. Os argumentos dessa função são a área em decimais (p) e os graus de liberdade da curva t (df). O resultado da função é o quantil.
...
A função `rchisq()` cria conjunto randomizado de números a partir de uma distribuição Chi quadrado. Os argumentos dessa função são: a quantidade de números e os graus de liberdade (degrees of freedom - df).
...
E também podemos usar o argumento add=TRUE para inserir várias curvas no mesmo gráfico.
...
Por padrão a função retorna probabilidade acumulada do valor da estatística ser menor ou igual a `q`. Caso deseje a probabilidade de ser **maior ou igual** a `q` é necessário acrescentar o argumento `lower.tail=FALSE`
...
O R possui essas funções de densidade, densidade cumulativa, quantil e números randomizados para diversas funções. O quadro abaixo resume esse capítulo.
