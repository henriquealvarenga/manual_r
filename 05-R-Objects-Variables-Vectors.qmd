...
Este capítulo usa duas funções de pacotes que serão apresentados mais adiante (@sec-packages): `recode_values()` do `dplyr` e `pluck()` do `purrr`. Os pacotes são carregados uma única vez, no início:
...
|  | Character | `character` | Texto ou string |
|  | Logical | `logical` | Valores lógicos (`TRUE` ou `FALSE`) |
|  | Complex | `complex` | Números Complexos |
|  | Raw | `raw` | Raw bytes |
| **Tipo Especial de dados** | Factor | `factor` | Dados categóricos |
| **Estruturas de Dados** | Vector | `vector` | Coleções homogêneas de tipos de dados básicos |
...
A ciência depende de dados, que são gerados a partir de alguma forma de coleta ou transformação de outros dados. Tudo que pode ser contado ou quantificado será armazenado numa variável. Uma variável deve ser entendida como um objeto que contém os resultados dessa coleta ou transformação de dados.

Mas dados podem ser coletados de diferentes modos: alguns dados são provenientes de algo que foi contado, outros dados provêm de algo que foi medido. Contar e medir fornecem diferentes tipos de dados. Podemos por exemplo, contar o número de pessoas com AIDS, o número de eleitores de um determinado político, o número de óbitos, de nascimentos etc. Por outro lado, podemos medir o perímetro cefálico de crianças recém-nascidas, o nível pressórico ou de glicemia de um grupo de pacientes, etc.
...
Algumas variáveis são nomes que expressam uma qualidade (que podem ter ou não uma ordenação). Em estatística essas variáveis são chamadas categóricas e podem se dividir em nominais ou ordinais. Os dados que não possuem nenhuma ordenação, são armazenados em variáveis categóricas nominais.

Variáveis categóricas nominais expressam qualidades, mas sem uma ordenação, tal como sexo, raça, local de nascimento etc.

Variáveis categóricas ordinais também expressam qualidades, mas têm uma ordenação, tais como grau de obesidade (leve, moderado, grave), estadiamento do câncer (estágio I, estágio II, estágio III) etc.

No R as variáveis categóricas são chamadas de `factor`, independente de serem nominais ou ordinais. Caso sejam variáveis categóricas ordinais, ou seja, caso exista uma ordem certa entre os elementos, existem meios de indicar isso para o R.

Frequentemente precisamos indicar ao R que uma determinada variável é categórica. Em algumas situações o R pode interpretar uma variável como numérica quando na verdade é categórica. É usual em pesquisas que os sexos sejam codificados com números, por exemplo 0 = Masculino, 1 = Feminino. Nesse caso, a variável `sexo` iria ser interpretada no R como um vetor numérico. Entretanto, os elementos desse vetor não representam números de verdade, mas são apenas símbolos para indicar o sexo masculino ou feminino.
...
No código abaixo você irá ver também um operador do R, o operador de atribuição: `<-` que indica que o que está à direita do operador será atribuído ao objeto à esquerda. Iremos detalhar mais sobre isso no capítulo adiante sobre operadores do R (@sec-operators).
...
Veja que para o R, tanto a variável `sexo` como a `idade` são numéricas.
...
Caso seja interessante renomear os elementos, transformando 0 em "Masc" e 1 em "Fem" podemos usar a função `recode_values()` do pacote `dplyr`. Iremos discutir esse pacote mais adiante, portanto, não se preocupe em entender o código abaixo por enquanto, veja apenas que podemos fazer isso facilmente com essa função.

```{r, message=FALSE, warning=FALSE}
# recodificando os elementos da variável sexo
df$sexo <- recode_values(df$sexo, "0" ~ "Masc",
                                  "1" ~ "Fem") |> as.factor()
...
Ou seja, podemos pedir ao R para contar o número absoluto ou a frequência relativa dos resultados. A função para tabular os dados de variáveis categóricas é `table()`.
...
A função `summary()` mostra um resumo dos valores: mínimo, primeiro quartil, mediana, média, terceiro quartil e máximo.
...
Variáveis lógicas são aquelas que armazenam resultados de operações lógicas. Operações lógicas são aquelas realizadas através de relacionais: igual, maior, maior ou igual, menor, menor ou igual, etc. Como já vimos, o resultado dessas operações não é um número, mas sim FALSO ou VERDADEIRO, e esse resultado pode ser armazenado numa variável. No exemplo uma operação lógica é realizada e seu resultado é colocado numa variável chamada teste.
...
##### Criando um vetor numérico com um determinado conjunto de números

```{r}
idades <- c(45, 32, 24, 23, 55, 56) 
```

##### Criando um vetor com uma sequência de números

```{r}
x <- c(1:10)
```

No exemplo acima vimos um novo operador `:` (dois pontos), que cria uma sequência de números. No capítulo sobre operadores do R (@sec-operators) iremos discutir melhor o que são operadores e quais os operadores do R.
...
# mostrando os resultados armazenados no novo vetor peso_kg
peso_kg
```
...
Poderíamos também ter feito tudo isso de forma mais simples numa única operação:
...
Para criar uma lista em R, utilizamos a função `list()`. Podemos incluir qualquer tipo e quantidade de elementos dentro de uma lista. Aliás, uma lista pode conter até mesmo outras listas. O exemplo abaixo é uma lista de pacientes, onde cada paciente é uma lista com diversos elementos.
...
Podemos usar esse mesmo operador `$` em sequência para acessar uma informação de uma lista que está dentro de outra lista. Por exemplo o sexo do paciente_1.
...
Enquanto a notação `$` é usada para acessar elementos nomeados, a notação `[[ ]]` é utilizada para acessar elementos individuais pelo nome (entre aspas) ou pela posição:

```{r}
# Acessando o mesmo elemento com a notação [[ ]]
pacientes[["paciente_1"]][["sexo"]]
```
...
Na seção de análises estatísticas você verá que muitas das análises têm como resultado uma lista e você precisará acessar os elementos dessa lista para obter as informações específicas que deseja, tal como, por exemplo, o valor de p. Vamos criar uma matriz de exemplo e realizar um teste do chi quadrado.
...
Observe que o resultado do teste chi-quadrado é uma lista. Para acessarmos o valor de p vamos precisar acessar essa lista usando o operador `$`. Podemos, se precisarmos, armazenar esse valor numa variável.
...
Uma variável é um objeto que armazena dados, tais como valores numéricos, datas, caracteres, palavras, valores lógicos etc. Uma variável é um objeto para armazenar um determinado tipo de dado de uma pesquisa. Se uma variável de minha pesquisa é a medida da pressão dos pacientes, posso no R criar uma variável chamada `pa`, `nivel.pressorico` ou `pressao.arterial` para armazenar esse conjunto de dados.
...
Entretanto, quando houver necessidade de usar nomes compostos, um modo adequado é usar um ponto separando as palavras, tais como: `glico.fem` ou `idade.media`.

Evite usar o underline em variáveis, a separação de palavras pelo underline é geralmente usado para nomear arquivos tais como: `research_results_fase_1.csv`.

Evite usar maiúsculas para separar as palavras de uma variável, pois esse estilo geralmente é usado para nomear funções, tal como em `solveEquation`. Veja que as palavras são separadas pelo uso de uma maiúscula no início das palavras, exceto a primeira. Esse modo de criar nomes é geralmente usado para nomearmos funções, portanto, vamos evitar fazer isso ao criarmos nomes de variáveis.
...
Veja o funcionamento dessas funções aplicadas a uma variável simples:
...
-   A variável `alcgp` indica a quantidade de álcool que o participante consome em gramas por dia e tem 4 níveis
