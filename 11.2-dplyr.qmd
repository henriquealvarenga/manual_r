...
Num data frame as linhas representam as diferentes observações e as colunas representam as variáveis. Por exemplo, em pesquisas na área de saúde, geralmente cada linha representa um participante da pesquisa e as colunas as variáveis da pesquisa.
...
```{r mpg-dictionary-dplyr, echo=FALSE, out.width='100%', fig.show='hold', fig.cap='mpg dictionary'}
knitr::include_graphics(rep("images/mpg.png", 1))
```

Sabemos que para acessar uma variável de um data frame usamos o operador `$` e que a variável com os nomes das montadoras é manufacturer. Vamos então checar quais são as montadoras dos carros desse pacote com o comando `unique()`. Essa função lista os elementos de um conjunto, sem repetição:

```{r}
unique(mpg$manufacturer)
```

Como podemos ver, esse banco de dados tem carros de várias montadoras. Em análises estatísticas é frequentemente necessário separar os dados de acordo com alguma regra, por exemplo, de acordo com a montadora. Podemos, por exemplo desejar analisar apenas os carros das montadoras tradicionais no Brasil (Ford, Chevrolet, Volkswagen). Vamos fazer um novo data frame com essas montadoras. Para isso usamos a função `filter()` como descrito anteriormente e criamos um novo data frame chamado `cfv`, letras iniciais dessas montadoras.
...
Podemos visualizar o consumo dessas três marcas selecionadas num boxplot comparativo, como feito a seguir:
...
Podemos usar o comando `filter()` para filtrarmos os dados de acordo com regras numéricas. Por exemplo, para sabermos quais são os carros que conseguem percorrer mais de 30 milhas com um galão usamos também a função `filter()` como abaixo, no qual criamos um novo data frame com o nome `bestcars`.

```{r}
bestcars <- filter(cfv, cty > 30)
```

Para mostrar o conteúdo desse novo data frame que chamamos de bestcars basta digitar o nome do data frame no console. E para saber apenas o nome dos modelos desses carros, basta usar o operador `$`, como mostrado abaixo:
...
A função `pull()` funciona de forma parecida com o operador `$`. Ambos têm a função de extrair valores de um data frame. Já vimos como usar o operador `$` para extrair valores de uma variável e usar esses valores em funções estatísticas tais como `mean()`, `median()` e `sd()`. Vamos relembrar como isso foi feito usando o dataset `mpg`.
...
Mas às vezes são necessárias diversas operações antes de encontrar o conjunto numérico desejado para calcular uma média. Por exemplo, se quisermos calcular a média das milhas percorridas com um galão em automóveis do tipo SUV? Vamos ver como podemos fazer isso usando o operador pipe `|>` e as funções `select()`, `filter()` e `pull()`.
...
O código acima não deu erro, mas devolveu `NA` acompanhado de um aviso. O motivo é que o argumento passado pelo pipe para a função `mean()` não é um vetor, mas sim um data frame (ou uma tibble). Podemos conferir isso no código abaixo, onde substituímos a função `mean()` pela função `class()` que informa o tipo de objeto.
...
Para que a função `mean()` possa calcular a média é necessário que o argumento passado para essa função seja um conjunto numérico, ou seja, um vetor numérico. É aí que entra a função `pull()`, que extrai os valores do data frame e repassa esses valores adiante pelo pipe. Vejamos agora como consertar o código.
...
O código acima pode ser lido da seguinte maneira: crie uma variável com o nome `cty.km` com valores iguais aos da variável `cty` multiplicado por 1.609344, proveniente do dataset `mpg`. Esse código, entretanto, tem um problema: o resultado dessa operação não foi salvo em nenhuma variável. Ao ser executado, esse código irá apenas mostrar todo o resultado sem salvar nada. Para que a nova variável seja salva é necessário acrescentar essa etapa no código como feito abaixo.

```{r}
mpg <- mutate(cty.km = cty*1.609344, mpg) 
```

Podemos também reescrever esse código usando o operador pipe `|>`, tornando esse código mais fácil de ser compreendido.
...
O código acima pode ser lido da seguinte forma: use o dataset `mpg`, crie uma variável com o nome `cty.km` com valores iguais aos da variável `cty` multiplicado por 1.609344. Finalmente, acrescente essa nova variável no dataset `mpg`.
...
```{r}
# Ordenar os dados pela coluna idade e depois pela coluna cidade
pacientes_ordenados_mult <- arrange(pacientes, idade, cidade)
pacientes_ordenados_mult
```

### Agrupando dados com `group_by()` e `by()`
...
-   **Integração com o pipe (`|>`)**: Permite a construção de pipelines de manipulação de dados de maneira clara e legível.
...
mpg |> 
group_by(class) |> 
summarize(media=mean(cty), mediana=median(cty), desvio_padrao=sd(cty), maximo=max(cty), minimo=min(cty))
```
...
Podemos resolver esse problema recodificando o valor da variável "Feminino" para "Fem" e "Masculino" para "Masc". Observe que é preciso atribuir o resultado a um objeto para que a recodificação seja salva.
...
Por exemplo, no dataset `mpg` a variável `trans` (tipo de transmissão) tem 10 categorias, ou seja, 10 diferentes tipos de transmissão: 8 tipos de marchas automáticas e 2 tipos de marchas manuais.

```{r}
unique(mpg$trans)
```

Para compararmos os carros com marchas automáticas com marchas manuais precisamos reduzir essas 10 categorias para apenas duas: `manual` e `automatica` (sem acento, seguindo a recomendação de não usar acentos em nomes).

Podemos fazer isso criando uma nova variável com apenas essas duas categorias.

Podemos fazer isso usando a função `mutate()` para criar uma nova variável `marcha`.
...
Veja que a variável marcha tem apenas 2 níveis: automatica e manual:
...
Ao trabalhar com dados, muitas vezes é necessário combinar vários data frames em um único data frame. As funções `bind_rows()` e `bind_cols()` do pacote `dplyr` são projetadas para concatenar data frames verticalmente, empilhando as linhas de cada data frame um em cima do outro, ou horizontalmente combinando suas colunas em um único data frame. Estas funções são particularmente úteis quando se tem dados provenientes de diferentes fontes ou partes de um estudo que precisam ser unificados.
...
```{r}
# Remover duplicatas com base na coluna id_paciente
pacientes_distintos_colunas <- distinct(pacientes, id_paciente, .keep_all = TRUE)
pacientes_distintos_colunas
```
